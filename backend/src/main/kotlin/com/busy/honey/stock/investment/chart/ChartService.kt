...
class ChartService (val stockPriceRepository: StockPriceRepository){
    ...
    fun getChart(stockId: Long, chartDto:ChartDto): List<Map<String, Any>>{
        ...
        while(startDate.compareTo(to) != 0){
            val endDate = LocalDateTime.parse(chartDto.from + " 23:59:59", DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))

            // 종가
            val closeStockPrice = stockPriceRepository.findStockPriceOrderByTimestampDesc(true, startDate, endDate, 1, 1)

            // 시가
            val openStockPrice = stockPriceRepository.findStockPriceOrderByTimestampAsc(true, startDate, endDate, 1, 1)

            // 최고가
            val highStockPrice = stockPriceRepository.findStockPriceOrderByPriceDesc(true, startDate, endDate, 1,1)

            // 최저가
            val lowStockPrice = stockPriceRepository.findStockPriceOrderByPriceAsc(true,  startDate, endDate, 1,1)

            // 체결된 거래량
            val volume = stockPriceRepository.countConcludedTrade(startDate, endDate)
